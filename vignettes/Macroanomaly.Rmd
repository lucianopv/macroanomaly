...
\begin{itemize}
  ...
  \item Hampel filter using \texttt{pracma::hampel()}, applied per country-indicator group sorted by time ("hampel")
\end{itemize}
```
## Example Data
...
The `macroanomaly` package provides a set of functions to detect anomalies in time series data, particularly focusing on macroeconomic indicators. The package allows the user to normalize, detrend, and deseasonalize the data, making it suitable for anomaly detection. The user can choose from several methods to detect anomalies, including Z-score based detection, Isolation Forests, Outlier Trees, IQR based outliers, Point and Collective Anomalies using the `anomaly` package, and the Hampel filter. The workflow is: `normalize()` → `detect()`. Multiple methods can be combined in a single `detect()` call, and the `outlier_indicator_total` column in the output summarises how many methods flagged each observation.
